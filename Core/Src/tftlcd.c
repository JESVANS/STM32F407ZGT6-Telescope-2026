...
void lcd_show_char(uint16_t x, uint16_t y, char ch, uint8_t size, uint8_t mode, uint16_t color)
{
    uint8_t temp, t1, t;
    uint16_t y0 = y;
    uint8_t csize = 0;
...
/**
 * @brief  显示 Image2LCD 生成的图片（带8字节文件头，RGB565格式）
 * @param  x      显示起始X坐标
 * @param  y      显示起始Y坐标
 * @param  image  Image2LCD 导出的数组指针（含8字节头：扫描模式、色彩模式、宽高等）
 * @note   Image2LCD 输出设置：水平扫描、16位真彩色(RGB565)、高位在前、包含图像头数据
 */
void lcd_show_image(uint16_t x, uint16_t y, const uint8_t *image)
{
    if (image == NULL) return;

    /* 解析 Image2LCD 8字节文件头 */
    uint16_t w = ((uint16_t)image[2] << 8) | image[3];  /* 宽度（大端） */
    uint16_t h = ((uint16_t)image[4] << 8) | image[5];  /* 高度（大端） */

    const uint8_t *p = image + 8;  /* 跳过8字节头，指向像素数据 */

    for (uint16_t j = 0; j < h; j++)
    {
        lcd_set_cursor(x, y + j);
        lcd_write_ram_prepare();

        for (uint16_t i = 0; i < w; i++)
        {
            /* RGB565 高字节在前 */
            uint16_t color = ((uint16_t)(*p) << 8) | (*(p + 1));
            p += 2;
            LCD->LCD_RAM = color;
        }
    }
